...
public class iso2709RecordFactory implements RecordBuilder, ApplicationContextAware, org.springframework.context.ApplicationListener, Serializable {
  ...
  */
  public Object createFrom(Document input_dom, String esn) throws RecordBuilderException {
    ...
    try {
      log.debug("open pip output stream");  //input_dom -> XMLSerializer -> PipedOutputStream -> PipedInputStream -> MarcXmlReader
      ...
      pos.flush();
      pos.close();
      log.debug("attempt to read marcxml from pipe");   //MarcXmlReader -> Record -> MarcStreamWriter -> Baos -> byteArray
      if (reader.hasNext()) {
        Record record = reader.next();
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        MarcWriter writer = new MarcStreamWriter(baos, "UTF-8");    //we only serialize to UTF-8
        writer.write(record);
  ...
  public Document getCanonicalXML(Object native_object) throws RecordBuilderException {
    ...
    try {
      MarcReader reader = new MarcStreamReader(new ByteArrayInputStream((byte[])native_object), "UTF-8");
